...
void MagicSlidingGenerator::initaliseSliderAttacks(PieceType pieceType){
    ...
    for (int index = 0; index < 64; index++){
        ...
        int maskBitCount = countBitsInBoard(mask);
        int occupancy_variations = 1 << maskBitCount;

        // loop over occupancy variations, init occupancies, magic index & attacks
        for (int count = 0; count < occupancy_variations; count++){
            if (pieceType == Bishop){
                ULL occupancy = setOccupancy(count, maskBitCount, mask);
                ULL magicIndex = occupancy * _bishop_magics[index] >> (64 - _bishop_relevant_bits[index]);
                _bishop_attacks[index][magicIndex] = bishopAttacksInitialisationStage(index, occupancy);
            }
            if (pieceType == Rook){
                ULL occupancy = setOccupancy(count, maskBitCount, mask);
                ULL magicIndex = occupancy * _rook_magics[index] >> (64 - _rook_relevant_bits[index]);
...
ULL MagicSlidingGenerator::setOccupancy(int index, int bitsInMask, ULL attackMask){
    ...
    for (int count = 0; count < bitsInMask; count++){
        int index = getIndexOfLeastSignificantBit(attackMask);
        attackMask = popBit(attackMask, index);
