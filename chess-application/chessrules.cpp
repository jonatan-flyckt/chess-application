...
bool ChessRules::pseudoLegalMoveCausedSelfCheck(const Move &move, const BitBoard &board){
    ...
    //if (move._origin_square == "e8" && move._destination_square == "f8"){
    //    cout << "------ BEFORE ------" << endl;
    //    printFullBitboard(board);
    //    cout << "--------------------" << endl;
    //}


    BitBoard resultingBoard;
    resultingBoard._en_passant_square = board._en_passant_square;
    resultingBoard._indices_of_bits_for_piece_types = board._indices_of_bits_for_piece_types;
    resultingBoard._white_king = board._white_king;
    resultingBoard._black_king = board._black_king;

    if (move._colour_performing_move == White){
        resultingBoard._white_pawns = board._white_pawns;
        resultingBoard._white_rooks = board._white_rooks;
        resultingBoard._white_knights = board._white_knights;
        resultingBoard._white_bishops = board._white_bishops;
        resultingBoard._white_queens = board._white_queens;

        if (move._piece._type == Pawn)
            resultingBoard._white_pawns = (board._white_pawns | destinationMask) & originComplementMask;
        if (move._piece._type == Knight)
            resultingBoard._white_knights = (board._white_knights | destinationMask) & originComplementMask;
        if (move._piece._type == Bishop)
            resultingBoard._white_bishops = (board._white_bishops | destinationMask) & originComplementMask;
        if (move._piece._type == Rook)
            resultingBoard._white_rooks = (board._white_rooks | destinationMask) & originComplementMask;
        if (move._piece._type == Queen)
            resultingBoard._white_queens = (board._white_queens | destinationMask) & originComplementMask;
        if (move._piece._type == King)
            resultingBoard._white_king = (board._white_king | destinationMask) & originComplementMask;

        if (move._move_type == EnPassant){
            resultingBoard._black_pawns = board._black_pawns & ~(1ULL<<(getIndexOfLeastSignificantBit(board._en_passant_square)-8));
        }
        else if (move._move_type == LongCastle){
            resultingBoard._white_king = _bit_masks[_index_from_square["c1"]];
            resultingBoard._white_rooks =
                    (board._white_rooks & _bit_masks_complement[_index_from_square["a1"]]) | _bit_masks[_index_from_square["d1"]];
        }
        else if (move._move_type == ShortCastle){
            resultingBoard._white_king = _bit_masks[_index_from_square["g1"]];
            resultingBoard._white_rooks =
                    (board._white_rooks & _bit_masks_complement[_index_from_square["h1"]]) | _bit_masks[_index_from_square["f1"]];
        }
        else if (move._move_type == Promotion || move._move_type == PromotionCapture){
            resultingBoard._white_pawns = resultingBoard._white_pawns & (destinationComplementMask ^ _bit_masks[_index_from_square[move._origin_square]]);
            if (move._promotion_selection == Queen)
                resultingBoard._white_queens = board._white_queens | destinationMask;
            else if (move._promotion_selection == Knight)
                resultingBoard._white_knights = board._white_knights | destinationMask;
            else if (move._promotion_selection == Bishop)
                resultingBoard._white_bishops = board._white_bishops | destinationMask;
            else if(move._promotion_selection == Rook)
                resultingBoard._white_rooks = board._white_rooks | destinationMask;
        }

        resultingBoard._black_pawns = board._black_pawns & destinationComplementMask;
        resultingBoard._black_rooks = board._black_rooks & destinationComplementMask;
        resultingBoard._black_knights = board._black_knights & destinationComplementMask;
        resultingBoard._black_bishops = board._black_bishops & destinationComplementMask;
        resultingBoard._black_queens = board._black_queens & destinationComplementMask;

    }
    else{
        resultingBoard._black_pawns = board._black_pawns;
        resultingBoard._black_rooks = board._black_rooks;
        resultingBoard._black_knights = board._black_knights;
        resultingBoard._black_bishops = board._black_bishops;
        resultingBoard._black_queens = board._black_queens;

        if (move._piece._type == Pawn)
            resultingBoard._black_pawns = (board._black_pawns | destinationMask) & originComplementMask;
        if (move._piece._type == Knight)
            resultingBoard._black_knights = (board._black_knights | destinationMask) & originComplementMask;
        if (move._piece._type == Bishop)
            resultingBoard._black_bishops = (board._black_bishops | destinationMask) & originComplementMask;
        if (move._piece._type == Rook)
            resultingBoard._black_rooks = (board._black_rooks | destinationMask) & originComplementMask;
        if (move._piece._type == Queen)
            resultingBoard._black_queens = (board._black_queens | destinationMask) & originComplementMask;
        if (move._piece._type == King)
            resultingBoard._black_king = (board._black_king | destinationMask) & originComplementMask;

        if (move._move_type == EnPassant){
            resultingBoard._white_pawns = board._white_pawns & ~(1ULL<<(getIndexOfLeastSignificantBit(board._en_passant_square)+8));
        }
        else if (move._move_type == LongCastle){
            resultingBoard._black_king = _bit_masks[_index_from_square["c8"]];
            resultingBoard._black_rooks =
                    (board._black_rooks & _bit_masks_complement[_index_from_square["a8"]]) | _bit_masks[_index_from_square["d8"]];
        }
        else if (move._move_type == ShortCastle){
            resultingBoard._black_king = _bit_masks[_index_from_square["g8"]];
            resultingBoard._black_rooks =
                    (board._black_rooks & _bit_masks_complement[_index_from_square["h8"]]) | _bit_masks[_index_from_square["f8"]];
        }
        else if (move._move_type == Promotion || move._move_type == PromotionCapture){
            resultingBoard._black_pawns = resultingBoard._black_pawns & (destinationComplementMask ^ _bit_masks[_index_from_square[move._origin_square]]);
            if (move._promotion_selection == Queen)
                resultingBoard._black_queens = board._black_queens | destinationMask;
            else if (move._promotion_selection == Knight)
                resultingBoard._black_knights = board._black_knights | destinationMask;
            else if (move._promotion_selection == Bishop)
                resultingBoard._black_bishops = board._black_bishops | destinationMask;
            else if(move._promotion_selection == Rook)
                resultingBoard._black_rooks = board._black_rooks | destinationMask;
        }

        resultingBoard._white_pawns = board._white_pawns & destinationComplementMask;
        resultingBoard._white_rooks = board._white_rooks & destinationComplementMask;
        resultingBoard._white_knights = board._white_knights & destinationComplementMask;
        resultingBoard._white_bishops = board._white_bishops & destinationComplementMask;
        resultingBoard._white_queens = board._white_queens & destinationComplementMask;

    }
    resultingBoard._all_black_pieces = resultingBoard._black_pawns | resultingBoard._black_rooks | resultingBoard._black_knights |
            resultingBoard._black_bishops | resultingBoard._black_queens | resultingBoard._black_king;
    resultingBoard._all_white_pieces = resultingBoard._white_pawns | resultingBoard._white_rooks | resultingBoard._white_knights |
            resultingBoard._white_bishops | resultingBoard._white_queens | resultingBoard._white_king;
    resultingBoard._all_pieces = resultingBoard._all_white_pieces | resultingBoard._all_black_pieces;
    _self_check_first_timer += nanosecond_measurement() - start;


    //if (move._origin_square == "e8" && move._destination_square == "f8"){
    //    cout << "------ AFTER ------" << endl;
    //    printFullBitboard(resultingBoard);
    //    cout << "--------------------" << endl;
    //}
    ...
    start = nanosecond_measurement();
    bool returnBool = move._colour_performing_move == White ? whiteKingIsInCheck(resultingBoard): blackKingIsInCheck(resultingBoard);
    _self_check_second_timer += nanosecond_measurement() - start;
